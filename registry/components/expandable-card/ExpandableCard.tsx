...
export function ExpandableCardList({ data }: ExpandableCardListProps) {
...
function CardItem({ 
  ...
  card: CardData; 
  onPress: (card: CardData, x: number, y: number, w: number, h: number) => void;
  hidden: boolean;
}) {
  const animatedRef = useAnimatedRef<{ measure: (callback: (x: number, y: number, width: number, height: number, pageX: number, pageY: number) => void) => void }>();

  const handlePress = () => {
    if (!animatedRef.current) return;
    
    animatedRef.current.measure((x: number, y: number, width: number, height: number, pageX: number, pageY: number) => {
      onPress(card, pageX, pageY, width, height);
    });
  };

  return (
    <Pressable onPress={handlePress} style={{ opacity: hidden ? 0 : 1 }}>
